...
export default function Home() {
  ...
  const [showPresentationViewer, setShowPresentationViewer] = useState(true);
        ...
        <aside className={`${sidebarCollapsed ? 'w-16' : 'w-80'} bg-white border-r border-gray-200 flex-shrink-0 transition-all duration-300 overflow-hidden hidden md:block`}>
          <div className={`${sidebarCollapsed ? 'p-2' : 'p-4'} space-y-2`}>
            ...
            {sidebarCollapsed ? (
              <a 
                href="/about-us"
                className="w-full p-3 rounded-lg hover:bg-gray-50 cursor-pointer transition-colors flex items-center justify-center no-underline"
                title="About Us"
              >
              ...
              <a 
                href="/about-us"
                className="w-full p-3 rounded-lg hover:bg-gray-50 cursor-pointer transition-colors flex items-center space-x-3 no-underline"
              >
                <svg className="w-5 h-5 text-indigo-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
